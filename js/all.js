...
function tooltip($el, sticky) {
    ...
    if ($el.data('tooltip-inline')) {
        ...
        $tooltip.css({
            left: $el.offset().left + ($el.outerWidth(false) / 2),
            top: $el.offset().top + $el.outerHeight(false) + 20,
            marginLeft: -($tooltip.outerWidth(false) / 2)
        })
    }
...
$(document).ready(function () {
    ...
    $body.on('click', '.toggleLogin', function (e) {
        ...
        if ($form.attr('action') == '/user/signup') {
            $h3.data('origTitle', $h3.contents().filter(function () {
                return this.nodeType === 3
            }).text()).text(Lang.log_into_chordify);
    ...
    $('body').on('submit', '.form-ajax', function (e) {
        ...
        $.ajax({
            url: $form.attr('action'), data: formData, dataType: 'json', method: 'post', success: function (data) {
    ...
    $body.on('click', 'a[href^="/?url="]', function (e) {
        ...
        $thumbnails.css({
            ...
            left: offset.left,
            zIndex: 100
        }).appendTo($body).animate({left: 0, top: 0, opacity: 0}, function () {
            $thumbnails.remove();
        });
