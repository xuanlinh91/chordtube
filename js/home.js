...
(function(){
	...
	$(document).ready(function(){
			...
			callback = function(items){
				...
				for( i in items ){
					...
					$rows = $rows.add(
						...
						.append('<span class="source">' + items[i].type + '</span>').
						data(items[i])
					);
				}
		...
		} else if( $urlInput.hasClass('is-running') ){
			startProgress();
			onUpload({status: 'Chordify!', process: 'init', progress: 0, total: 0});
		}
		...
		if( Chordify.getParameter('welcome') ){
			/*Checkout.showPopup();
			$('.popup h2').text(Lang.welcome);
			$('.popup h3').text(Lang.go_premium_more);
			$('.popup .checkout-products').before('<p style="text-align:center;margin-top:-25px"><a href="#close" onclick="hidePopup();return false;">' + Lang.continue_free + '</a></p>');
			*/
